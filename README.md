...
| **Поддерживаемые длины ключей** | 256 бит, 512 бит                                            |
| **Хэш-функция**                 | ГОСТ Р 34.11-2012 (Streebog)                                |
| **Размер подписи**              | 512 бит (для 256-битного ключа), 1024 бит (для 512-битного) |
| **Стойкость**                   | Основана на сложности ECDLP                                 |
| **Скорость работы**             | Высокая (благодаря эффективным операциям на ЭК)             |
| **Стандартизация**              | Российский национальный стандарт (ГОСТ)                     |
| **Рекомендуемые кривые**        | id-tc26-gost-3410-2012-256-paramSetA (256 бит)              |
|                                 | id-tc26-gost-3410-2012-512-paramSetA (512 бит)              |
| **Применение**                  | Юридически значимая ЭП, защита информации                   |


| Характеристика               | Значение для 256-бит                 | Значение для 512-бит                 |
|------------------------------|--------------------------------------|--------------------------------------|
...
| **Хэш-функция**              | Streebog256                          | Streebog512                          |
...
**Результат:**  
```
Ключевая пара успешно сгенерирована (PUBLIC: 'public.key', PRIVATE: 'private.key')!
```
